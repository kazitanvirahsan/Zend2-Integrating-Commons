...
class AlbumTest extends PHPUnit_Framework_TestCase{
    ...
    public function testExchangeArraySetsPropertyCorrectly() {
        $album = new Album();
        ...
        $this->assertSame($data['artist'] , $album->artist , '"artist" was  not set properly');
        $this->assertSame($data['id'] , $album->id , '"id" was  not set properly');
        $this->assertSame($data['title'] , $album->title , '"title" was  not set properly');
        
        
    }
    
    public function testExchangeArrayPropertiesToNullIfKeysAreNotPresent(){
        $album = new Album();
